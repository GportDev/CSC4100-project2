...
static bool wakeup_time_less (const struct list_elem *a,
...
timer_sleep (int64_t ticks_to_sleep)
{
  ...
  int64_t wakeup_tick = timer_ticks () + ticks_to_sleep;
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  /* Per-tick bookkeeping in threads (statistics, time slice) */
  thread_tick ();

  /* --- MLFQS periodic updates ---
     - Every TIMER_FREQ ticks (1 second): update load_avg and recent_cpu for all threads.
     - Every 4 ticks: recalculate priorities for all threads.
     Note: thread_tick() already increments recent_cpu for the running thread when MLFQS is enabled.
  */
  if (thread_mlfqs)
    {
      ...
      /* Every 4 ticks, recompute priorities. This may change the ready list order
         and could require a context switch. Request a yield on return from interrupt. */
      if (ticks % 4 == 0)
        {
          thread_recalculate_priority_all ();
          /* Request preemption after interrupt handling if scheduler desires it. */
...
real_time_sleep (int64_t num, int32_t denom)
{
  ...
  */
  int64_t tics = num * TIMER_FREQ / denom;
...
/** Comparison function for sorting sleeping threads by wakeup time.
